...
export const qqzifetch =async function(options) {
  ...
  if (options.url.indexOf('http') === -1) {
    if (options.url[0] === '/') options.url = options.url.substring(1)
    options.url = static_app__domain + options.url
  }
  if (!options.data) options.data = {}

  //var user = this.modules.user;
  //if (user) options.data["qqzitoken"] = user.token;
  var fetchOptions = {}

  fetchOptions.method = options.type
  var dataStr = ''
  var isSaveAction = options.url.indexOf('DoSave') > -1
  if (options.data !== null) {
    if (options.data['qqzitoken'])
      options.data['qqzitoken'] = encodeURIComponent(options.data['qqzitoken'])
    for (var key in options.data) {
      if (options.data[key] === null) options.data[key] = ''
      if (isSaveAction) {
        dataStr += key + '=' + options.data[key] + '&'
      } else dataStr += key + '=' + encodeURIComponent(options.data[key]) + '&'
    }
    dataStr = dataStr.substring(0, dataStr.length - 1)
  }
  if (options.type === 'GET') {
    if (options.data !== null && dataStr.length > 0) {
    ...
    if (typeof FormData !== 'undefined') {
      ...
      if (options.from) {
        for (var key in options.data) {
          form.append(key, options.data[key]) //encodeURIComponent(options.data[key]));
        }
      } else {
        for (var key in options.data) {
          if (isSaveAction) {
...
export const getNologinSessionId = () => {
  return fetch(static_app__domain + 'Home/NewGuid').then(response =>
    response.json().then(response => response.Result)
  )
}
export const enURL = (url, data) => {
  let step = ''
  for (let key in data) {
    step += `${key}=${encodeURIComponent(data[key])}&`
  }
  step = step.substring(0, step.length - 1)
  if (url.indexOf('?') > -1) {
    url = `${static_app__domain}${url}&${step}`
  } else {
    url = `${static_app__domain}${url}?${step}`
  }
  return url
...
export const setUrlMd5 =async function(url) {
  ...
  var nologin_sessionId = await AsyncStorage.getItem('nologin_sessionId');
    ...
    if (nologin_sessionId) {
      ...
      else url += '&sessionId=' + nologin_sessionId
      hasData = true
    }
    return url
  }
  ...
  var temp = url.substring(static_app__domain.length - 1) + user.id + user.secret + times
  var md5text = md5(temp)
  //var pos1= options.url.indexOf('qqzitoken');
  var addStr = 'secret=' + md5text + '&ts=' + times
...
export const encode = ss => {
  ...
  var x = []
  var padchar = map[64]
  var len = ss.length - ss.length % 3

  for (var i = 0; i < len; i += 3) {
    b = (ss.charCodeAt(i) << 16) | (ss.charCodeAt(i + 1) << 8) | ss.charCodeAt(i + 2)
  ...
  switch (ss.length - len) {
    case 1:
      b = ss.charCodeAt(i) << 16
      x.push(map.charAt(b >> 18) + map.charAt((b >> 12) & 0x3f) + padchar + padchar)
      break
    case 2:
      b = (ss.charCodeAt(i) << 16) | (ss.charCodeAt(i + 1) << 8)
      x.push(
        map.charAt(b >> 18) + map.charAt((b >> 12) & 0x3f) + map.charAt((b >> 6) & 0x3f) + padchar
      )
      break
    default:
      break
